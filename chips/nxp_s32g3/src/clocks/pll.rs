...
/// PLL lock timeout in milliseconds.
///
/// Per RM §24.5.3, PLL lock time is typically < 100 µs. We allow 5 ms as a
/// generous upper bound to accommodate worst-case silicon and voltage/temp.
const PLL_LOCK_TIMEOUT_MS: u32 = 5;
...
register_structs! {
    ...
    pub PllRegisters {
        ...
        (0x01C => _reserved1),
        /// PLL Clock Mux Register (PLLCLKMUX) — selects reference clock (RM §27.5.6: offset 0x20)
...
register_bitfields![u32,
    ...
    /// PLL Divider Register (RM PLLDV: RDIV at 12-14, MFI at 0-7)
    PLLDV [
    ...
    /// PLL Fractional Divider (RM PLLFD: SDMEN at 30, MFN at 0-14)
    PLLFD [
...
impl Pll {
    ...
    /// down (PLLPD=1).
    ///
    /// If the PLL is already locked (hardware LOCK=1, e.g. set by bootloader),
    /// this is a no-op.
    ///
    /// # Errors
    ...
    ///
    /// If the PLL is already locked (e.g. configured by the bootloader),
    /// returns immediately.
    ///
    /// Uses the DWT cycle counter for a wall-clock bounded timeout
    /// ([`PLL_LOCK_TIMEOUT_MS`] ms at FIRC 48 MHz). Returns an error rather
    /// than spinning forever if the PLL fails to lock.
    ///
    /// # Errors
    /// - [`ErrorCode::BUSY`]: PLL did not lock within the timeout
    pub fn enable_pll(&self) -> Result<(), ErrorCode> {
        ...
        dwt_enable();
        let timeout_cycles = ms_to_cycles(PLL_LOCK_TIMEOUT_MS);

        // Power up: clear PLLPD
        ...
        loop {
            ...
            if elapsed >= timeout_cycles {
                debug!(
                    "[{}] TIMEOUT after {} cycles ({} ms)",
                    name, elapsed, PLL_LOCK_TIMEOUT_MS
                );
                // Re-assert power-down to leave the PLL in a known state.
    ...
    ///
    /// # Errors
    /// - [`ErrorCode::BUSY`]: PLL did not power down within the timeout
    pub fn disable_pll(&self) -> Result<(), ErrorCode> {
        ...
        loop {
            ...
            if dwt_elapsed_since(start) >= timeout_cycles {
                return Err(ErrorCode::BUSY);
            }
        }
    }
